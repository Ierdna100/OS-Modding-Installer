...
function main() {
    ...
    if (!fs.existsSync(path.join(outputPath, "installData")))
        fs.mkdirSync(path.join(outputPath, "installData"));
    if (!fs.existsSync(path.join(outputPath, "installData", "assemblies")))
        fs.mkdirSync(path.join(outputPath, "installData", "assemblies"));
    if (!fs.existsSync(path.join(outputPath, "installData", "OSLoaderContents")))
        fs.mkdirSync(path.join(outputPath, "installData", "OSLoaderContents"));
    ...
    for (const file of fs.readdirSync("./unity2019.4.40f1")) {
        if (!fs.lstatSync(path.join("./unity2019.4.40f1", file)).isFile())
            continue;
        if (fs.existsSync(path.join(outputPath, "installData/assemblies", file)))
            continue;
    ...
    fs.copyFileSync(path.join("../ProjectReferences/Assets/StreamingAssets/loader.manifest"), path.join(outputPath, "installData/OSLoaderContents/loader.manifest"));
